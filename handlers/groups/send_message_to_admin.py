...
async def create_topic(message: types.Message):
    ...
    for group in (FOR_MAN_ADMINS, FOR_WOMAN_ADMINS):
        ...
        if not await db.select_topic(topic_created_group_id=group, for_purpose="accepted_questions"):
            data = await bot.create_forum_topic(chat_id=group, name="Kelib tushgan savollar")
            await db.add_topic(
                topic_id=data.message_thread_id,
                topic_name=data.name,
                topic_created_group_id=group,
                created_at=await get_now(),
                for_purpose="accepted_questions"
            )

        # for answered questions
        if not await db.select_topic(topic_created_group_id=group, for_purpose="answered_questions"):
            data = await bot.create_forum_topic(chat_id=group, name="Javob berilgan savollar")
            await db.add_topic(
                topic_id=data.message_thread_id,
                topic_name=data.name,
                topic_created_group_id=group,
                created_at=await get_now(),
                for_purpose="answered_questions"
            )
